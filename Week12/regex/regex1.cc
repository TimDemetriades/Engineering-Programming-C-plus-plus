...
int main() {
  ...
  while (regex_search (s,m,r)) {    // s = string we are looking in, m = match, r = regex expression
                                    // returns True if found
    for (auto x : m)
			cout << x << "\t";
    cout << endl;
    s = m.suffix().str();           // after finding string, set s to after m to continue searching (inneficient)
  }
}
